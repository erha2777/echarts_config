    ...
    series: [
        {
            ...
            // 要绘制的“云”的形状。
            // 可以用回调函数，或者一个形状的呈现。 
            // 可用的形状是 circle 圆形的(默认)、cardioid 心形曲线(苹果或心形曲线，最著名的极坐标方程)、diamond 菱形(正方形的别名)、triangle-forward 三角形正三角形、triangle 三角形、triangle-upright 直角三角形、pentagon 五边形、star 星形
            shape: 'circle',
            // 保持maskImage的纵横比或形状的1:1
            keepAspect: false,
            // 白色区域将被排除在绘制文本之外的剪影图像,随着云形状的增长，形状选项将继续适用
            // maskImage: maskResource,
            // 下面的左/顶/宽/高/右/底用于定位单词云
            // 默认放置在中心，大小为75% x 80%。 
            left: 'center',
            top: 'center',
            width: '70%',
            ...
            // 默认最小12px，最大60px大小。 
            sizeRange: [6, 78],
            // 文字旋转范围和步进度。 文本将被rotationStep 45在[- 90,90]范围内随机旋转 
            rotationRange: [-45, 90],
            // rotationStep: 45,
            // 网格(以像素为单位)的大小，用于标记画布的可用性,网格尺寸越大，字与字之间的间距就越大
            gridSize: 8,
            // 设置为true允许文字部分绘制在画布外。允许文字比要绘制的画布大 
            drawOutOfBound: false,
            // 如果执行布局动画,如果执行layout animationNOTE禁用它会导致UI阻塞当有很多字
            layoutAnimation: true,
            // 全局文本样式
            textStyle: {
                fontFamily: 'sans-serif',
                fontWeight: 'bold',
                // 全局文本styleColor可以是回调函数或颜色字符串 
                color: function () {
                    // 随机颜色
                    return 'rgb(' + [
        ...
        <div class="config">
            ...
            <div class="config-item">
                ...
                <el-radio-group v-model="option.series[0].shape">
                    ...
                    <el-radio label="cardioid">cardioid</el-radio>
                    <el-radio label="diamond">diamond</el-radio>
                    <el-radio label="triangle-forward">triangle-forward</el-radio>
                    <el-radio label="triangle">triangle</el-radio>
                    <el-radio label="triangle-upright">triangle-upright</el-radio>
                    <el-radio label="pentagon">pentagon</el-radio>
                    <el-radio label="star">star</el-radio>
                </el-radio-group>
            </div>
            <div class="config-item">
                keepAspect:
                <el-radio-group v-model="option.series[0].keepAspect">
            ...
            <div class="config-item">
                sizeRange: <br>
                min：<el-input-number v-model="option.series[0].sizeRange[0]"  controls-position="right"/><br>
                max：<el-input-number v-model="option.series[0].sizeRange[1]"  controls-position="right"/>
            </div>
            <div class="config-item">
                rotationRange: <br>
                min：<el-input-number v-model="option.series[0].rotationRange[0]"  controls-position="right"/><br>
                max：<el-input-number v-model="option.series[0].rotationRange[1]"  controls-position="right"/>
            </div>
            <div class="config-item">
                gridSize: 
                <el-input-number v-model="option.series[0].gridSize"  controls-position="right"/><br>
            </div>
            <div class="config-item">
                drawOutOfBound: 
                <el-radio-group v-model="option.series[0].drawOutOfBound">
                    <el-radio :label="true">是</el-radio>
                    <el-radio :label="false">否</el-radio>
                </el-radio-group>
            </div>
            <div class="config-item">
                layoutAnimation: 
                <el-radio-group v-model="option.series[0].layoutAnimation">
                    <el-radio :label="true">是</el-radio>
                    <el-radio :label="false">否</el-radio>
                </el-radio-group>
            </div>
